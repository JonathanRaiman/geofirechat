...
var MessageView = Backbone.View.extend({
  ...
  , handleNewMessage: function(data) {
  	...
  	var seconds = pad2(currentTime.getSeconds());
  	var minutes = pad2(currentTime.getMinutes());
  	var hours = pad2(currentTime.getHours());
  	var month = pad2(currentTime.getMonth() + 1);
	var day = pad2(currentTime.getDate());
  ...
  , render: function() {
    ...
    var data = messages.map(function(message) {
    	...
    	return message.get('receptiontime') + message.get('content') + '</br><a>' + message.get('cityname')+ '</a></br>' 
    	});
