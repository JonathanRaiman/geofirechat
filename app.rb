...
post '/messages' do
  ...
  message = JSON.parse(request.body.read.to_s).merge(:id => @@count +=1, :receptiontime => time.strftime("%H:%M:%S"), :user_ip => request.ip, :cityname => (jsonresponse['cityName']).capitalize!, :latitude => jsonresponse['latitude'], :longitude => jsonresponse['longitude'])
  @@data << message
  message.to_json
end