...
post '/messages' do
  ...
  message = JSON.parse(request.body.read.to_s).merge(:id => @@count +=1, :user_ip => request.ip, :cityname => jsonresponse['cityName'], :latitude => jsonresponse['latitude'], :longitude => jsonresponse['longitude'])
  @@data << message
  message.to_json
end